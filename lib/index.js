  ...
  try {
    ...
    for (const line of raw.split("\n")) {
      try {
        ...
        const type = rec?.type;
        const msg = type?.endsWith("/message") ? rec.message : rec?.message;
        if (firstUser === null && msg?.role === "user" && Array.isArray(msg.content)) {
...
function normalizeRecord(rec) {
  const type = rec?.type;
  const msg = type?.endsWith("/message") ? rec.message : (rec?.message ?? rec);
